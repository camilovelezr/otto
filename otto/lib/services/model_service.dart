...
String generateObjectId() {
  ...
  final timestamp = (DateTime.now().millisecondsSinceEpoch ~/ 1000)
      .toRadixString(16)
      .padLeft(8, '0');
  final machineId = (123456)
      .toRadixString(16)
      .padLeft(6, '0'); // Replace with a random number
  final processId =
      (DateTime.now().microsecond % 65535).toRadixString(16).padLeft(4, '0');
  final counter = (DateTime.now().millisecondsSinceEpoch % 16777216)
      .toRadixString(16)
      .padLeft(6, '0');
...
class ModelService {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          debugPrint(
              'Models response body: ${response.body.substring(0, response.body.length > 100 ? 100 : response.body.length)}...');

          if (data is List) {
            final modelsList =
                data.map((json) => LLMModel.fromJson(json)).toList();
            debugPrint('Parsed ${modelsList.length} models successfully');

            // Print out all model IDs for debugging
            if (modelsList.isNotEmpty) {
              debugPrint(
                  'Found ${modelsList.length} models from /models/list endpoint');
              modelsList.forEach(
                  (model) => debugPrint('Available model: ${model.modelId}'));
            }
      ...
      if (response.statusCode == 404) {
        ...
        debugPrint(
            'Authentication error when fetching models: ${response.body}');
        return [];
      }
  ...
  Future<LLMModel> getModel(String modelId) async {
    try {
      ...
      throw Exception(
          'Failed to fetch model: ${response.statusCode} - ${response.body}');
    } catch (e) {
  ...
  Future<List<LLMModel>> syncModels({String? userId, String? username}) async {
    ...
    final effectiveUserId =
        (userId == null || userId.isEmpty) ? defaultObjectId : userId;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        try {
          final data = json.decode(response.body);
          debugPrint(
              'Sync response body: ${response.body.substring(0, response.body.length > 100 ? 100 : response.body.length)}...');

          if (data is List) {
            final modelsList =
                data.map((json) => LLMModel.fromJson(json)).toList();
            debugPrint(
                'Parsed ${modelsList.length} models successfully after sync');
            return modelsList;
            ...
            if (modelsData is List) {
              final modelsList =
                  modelsData.map((json) => LLMModel.fromJson(json)).toList();
              debugPrint(
                  'Parsed ${modelsList.length} models successfully after sync (from nested list)');
              return modelsList;
            }
          } else {
            ...
            debugPrint(
                'Sync response unexpected, falling back to GET /models/list');
        ...
        debugPrint(
            'Failed to sync models: ${response.statusCode} - ${response.body}');
    ...
    debugPrint(
        'Sync models function reached unexpected end, falling back to getModels');
    return await getModels();
  }
